...
interface AARItem {
  id: string; summary: string | null; mission_date: string | null; mission_type: string | null;
  unit_designation: string | null; status: string; created_at: string;
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">{total} AAR{total !== 1 ? 's' : ''} completed</p>
        </div>
        <Link href="/dashboard/debrief"
          className="inline-flex items-center gap-2 px-4 py-2.5 bg-green-700 hover:bg-green-600 text-white text-sm font-medium rounded-lg transition-colors">
      ...
      <div className="relative mb-6">
        ...
        <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}
          ...
          className="w-full pl-10 pr-4 py-2.5 bg-gray-900 border border-gray-800 rounded-lg text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-green-500/40" />
      </div>

      {loading ? (
        <div className="flex items-center justify-center h-48"><Loader2 className="w-6 h-6 text-green-400 animate-spin" /></div>
      ) : aars.length === 0 ? (
        <div className="text-center py-16 border border-dashed border-gray-800 rounded-xl">
          <MessageSquare className="w-10 h-10 text-gray-700 mx-auto mb-3" />
          <h3 className="text-sm font-medium text-gray-400 mb-1">{search ? 'No matching AARs' : 'No AARs yet'}</h3>
          <p className="text-xs text-gray-600 mb-4">{search ? 'Try different keywords.' : 'Run your first AAR with First Sergeant.'}</p>
          {!search && (
            <Link href="/dashboard/debrief" className="inline-flex items-center gap-2 px-4 py-2 bg-green-700 hover:bg-green-600 text-white text-sm font-medium rounded-lg transition-colors">
              <MessageSquarePlus className="w-4 h-4" />Run AAR
            </Link>
          )}
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      {aar.mission_type && <span className="text-xs font-mono px-1.5 py-0.5 rounded bg-gray-800 text-gray-400">{aar.mission_type}</span>}
                      {aar.mission_date && <span className="text-xs text-gray-600">{formatDate(aar.mission_date)}</span>}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={cn('text-xs px-2 py-0.5 rounded flex items-center gap-1',
                      aar.status === 'final' ? 'bg-emerald-900/30 text-emerald-400'
                        : aar.status === 'review' ? 'bg-green-900/30 text-green-400'
